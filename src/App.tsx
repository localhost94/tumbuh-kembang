...
import { IconAlert, IconArrow, IconChart, IconShield, IconSpark, IconSyringe } from './components/Icons'
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
          <a href="#top" className="flex items-center gap-2.5">
            ...
            <span className="leading-tight">
              <span className="block text-sm font-extrabold text-teal-950">Tumbuh Kembang Anak</span>
              <span className="block text-[11px] font-medium text-teal-600">Panduan Interaktif · Buku KIA 2024</span>
            </span>
          </a>
          <nav className="hidden items-center gap-1 md:flex">
            {NAV.map((n) => (
              <a
                key={n.href}
                href={n.href}
                className="rounded-full px-3.5 py-2 text-sm font-semibold text-teal-700 transition hover:bg-teal-50"
              >
                {n.label}
              </a>
            ))}
          ...
          <a
            href="#pertumbuhan"
            className="hidden rounded-full bg-teal-600 px-4 py-2 text-sm font-semibold text-white shadow-md shadow-teal-600/25 transition hover:bg-teal-700 sm:inline-flex"
          >
            Cek Pertumbuhan
          </a>
        </div>
      ...
      <main id="top">
        ...
        >
          <div className="space-y-6">
            <AgeTimeline month={month} onChange={setMonth} />
        ...
        >
          <div className="mb-5 flex flex-wrap items-center gap-3">
            <span className="inline-flex items-center gap-2 rounded-full bg-cyan-50 px-3.5 py-2 text-sm font-semibold text-cyan-800 ring-1 ring-cyan-100">
              <IconSyringe className="h-4 w-4" /> Usia terpilih: {month} bulan
            </span>
            <a href="#usia" className="text-sm font-semibold text-teal-700 underline decoration-teal-300 underline-offset-4">
    ...
    <section className="relative mx-auto max-w-6xl px-4 pt-12 pb-4 sm:px-6 sm:pt-16">
      <div className="grid items-center gap-10 lg:grid-cols-[1.1fr_0.9fr]">
        <div>
          ...
          <h1 className="mt-5 text-4xl leading-[1.05] font-extrabold tracking-tight text-teal-950 sm:text-5xl lg:text-6xl">
            Pendamping visual{' '}
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            <a
              href="#usia"
              className="inline-flex items-center gap-2 rounded-full bg-teal-600 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-teal-600/25 transition hover:bg-teal-700"
            >
              Mulai jelajah per usia <IconArrow className="h-4 w-4" />
            </a>
            <a
              href="#pertumbuhan"
              className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-bold text-teal-700 ring-1 ring-teal-200 transition hover:bg-teal-50"
            >
...
function Stat({ value, label }: { value: string; label: string }) {
  return (
    <div>
      ...
      <p className="text-xs font-semibold tracking-wide text-teal-900/60 uppercase">{label}</p>
    </div>
  )
}
...
function HeroVisual() {
  return (
    <div className="relative">
      <div className="rounded-[2rem] border border-teal-100 bg-white/95 p-6 shadow-[0_30px_70px_-40px_rgba(13,148,136,0.6)]">
        <svg viewBox="0 0 360 300" className="h-auto w-full">
          ...
          {[
            ...
            <circle key={cx} cx={cx} cy={cy} r="7" fill="#fff" stroke="#0d9488" strokeWidth="4" />
          ))}

          <circle cx="40" cy="52" r="26" fill="#fde68a" opacity="0.7" />
          <circle cx="318" cy="64" r="16" fill="#99f6e4" />
          <rect x="296" y="196" width="34" height="34" rx="10" fill="#fda4af" opacity="0.6" />
        </svg>
        ...
        <div className="mt-4 grid grid-cols-3 gap-3 text-center">
          {[
            ...
            <div key={c.label} className="rounded-2xl bg-teal-50/80 px-2 py-3">
              <p className="text-[11px] font-semibold tracking-wide text-teal-600 uppercase">{c.label}</p>
              <p className="text-sm font-extrabold text-teal-900">{c.value}</p>
            </div>
          ))}
        </div>
      </div>
      <div className="absolute -bottom-5 -left-4 hidden rounded-2xl bg-white px-4 py-3 shadow-lg ring-1 ring-teal-100 sm:block">
      ...
      <div className="absolute -top-5 -right-3 hidden rounded-2xl bg-white px-4 py-3 shadow-lg ring-1 ring-teal-100 sm:block">
        <p className="flex items-center gap-2 text-xs font-bold text-teal-800">
          <IconAlert className="h-4 w-4 text-rose-500" /> Deteksi dini
        </p>
      </div>
    </div>
  )
  ...
  return (
    <div className="rounded-3xl border border-teal-100/80 bg-white/95 p-6 shadow-[0_10px_40px_-24px_rgba(13,148,136,0.45)]">
      <div className="mb-4 flex items-center gap-3">
        ...
        <h3 className="text-base font-bold text-teal-950">{title}</h3>
      </div>
      ...
      <div className="mt-5 grid grid-cols-2 gap-2">
        {EMERGENCY_NUMBERS.map((e) => (
          <div key={e.label} className="rounded-2xl bg-rose-50 px-3 py-2 text-center ring-1 ring-rose-100">
            <p className="text-lg font-extrabold text-rose-700">{e.number}</p>
            <p className="text-[11px] font-semibold text-rose-500">{e.label}</p>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1.4fr_1fr]">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-extrabold text-teal-950">Tumbuh Kembang Anak</span>
          </div>
          <p className="mt-4 max-w-2xl text-sm leading-relaxed text-teal-900/70">{DISCLAIMER}</p>
          <div className="mt-6 flex flex-wrap gap-3">
            <a
              href={SOURCE.url}
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 rounded-full bg-teal-50 px-4 py-2 text-xs font-semibold text-teal-700 ring-1 ring-teal-100 transition hover:bg-teal-100"
            >
        ...
        <div className="rounded-3xl border border-teal-100 bg-white/80 p-5">
          <h3 className="text-xs font-bold tracking-wide text-teal-600 uppercase">Sumber data</h3>
          <p className="mt-2 text-sm font-bold text-teal-950">{SOURCE.title}</p>
          <ul className="mt-3 space-y-1.5 text-xs leading-relaxed text-teal-900/70">
            <li>{SOURCE.publisher}</li>
            <li>{SOURCE.isbn}</li>
            <li>Tahun {SOURCE.year}</li>
          </ul>
          <div className="mt-4 flex items-center gap-2 border-t border-teal-100 pt-4 text-xs text-teal-900/60">
            <IconChart className="h-4 w-4 text-teal-500" />
            Data gizi mengacu pada Permenkes No. 2 Tahun 2020 tentang Standar Antropometri Anak.
          </div>
        </div>
      </div>
      <div className="border-t border-teal-100 py-5 text-center text-xs text-teal-900/50">
